...
| IME Compose 嵌入（仅外层包裹，内部未迁移） | `input/LifecycleInputMethodService.kt`(owner 链：Lifecycle + SavedStateRegistry + ViewModelStore 统一挂到 IME `window.window.decorView`) + `FcitxInputMethodService.createComposeInputView()`(用工厂函数建 `ComposeView` 作 IME 输入视图根，`setContent` 内 `AndroidView` 承载原 `InputView`)；重建由 `themeState` / `recreateNonce` 驱动 `key()` 重挂载，等价原 `replaceInputView`。`InputView`/`CandidatesView` 内部 View 实现零改动，`CandidatesView` 仍挂 `contentView`；后续全 Compose 迁移可直接把 `AndroidView` 内容换成 Composable |
| WebDAV 云端备份同步(偏好 zip + 词库逐文件) | `sync/webdav/*`(Config/DictCollector+指纹/BackupZips/WebDavSyncEngine/SyncRestorer/AutoDictSync/DictReload); UI `ui/main/compose/screens/WebDavSyncScreen.kt`(服务器登录走弹窗 `WebDavLoginDialog`,云端目录固定为服务器根/fcitx5xzy); 数据管理(浏览/导出/导入/WebDAV 入口)直接内联在 `ComposeManagedPrefsScreen.kt` 的 Advanced 分类列表底部; 偏好 zip 兼容 `UserDataManager.import` 分区(metadata 硬校验/分区软容忍,走 `UserDataImportCompat`); 词库改为逐文件同步(不再打整包 zip)：上传 `uploadDictFiles`、下载 `downloadDictFiles`,按 `data/` 下相对路径映射到 `fcitx5xzy/dict/<相对路径>`,内容摘要/远端 mtime 快照做增量;`DictReload` 区分 pinyin/quickphrase(引擎热重载,无需重启)与 table(无热重载接口,必须重建进程); 词库自动同步=事件+常驻驱动(无 WorkManager),锚点仅加在 FcitxApplication/IME service 与 PinyinDictManager/TableManager/QuickPhraseManager/ClipboardDictFeeder; 连接配置存 `filesDir` 私有文件(不入 shared_prefs,自指不随备份); Manifest 已加 INTERNET/ACCESS_NETWORK_STATE + `res/xml/network_security_config.xml` |
| 偏好备份/恢复忽略广播配对 | 配对记录存 Room `databases/broadcast_db`(`PairedAppEntity.keyAlias` 指向本机 Keystore 密钥，由配对码建立、不可迁移)；备份 zip 打 `databases` 分区时经 `BroadcastBackupFilter` 剔除该库(`UserDataManager.export`/`BackupZips.buildPrefsZip`)；恢复导入(`UserDataManager.import`/`UserDataImportCompat.import`)只丢弃 zip 内携带的配对库、**不动本机已有有效配对**(本机 Keystore 密钥仍在可正常工作) |
