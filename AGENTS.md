...
- **ComposeView 是 final 类**：不要把键盘 Compose 宿主写成子类，用 `createKeyboardMiuixOverlayHost()` 工厂(位于 `input/KeyboardMiuixOverlayHost.kt`);`setContent` 是它的**成员函数**，不需要 import。
- **键盘 View→Compose 单向桥**：键盘里的 miuix 弹层/菜单/snackbar 只能经 `KeyboardMiuixBridge` 请求,宿主挂在 `InputView.init` 末尾(popup.root 之上)。
