...
| 数字键盘横屏分体历史符号 | `NumberKeyboard.kt`(分体分支 buildSplitLayout/buildRow4Split) + `RecentSymbolsView.kt`(历史符号面板)；左侧 50% 为「!?#切换钮 + 4 列可滚动网格」历史符号，右侧 50% 保留「符号滑块+9宫格」且 row4 无 `!?#`（逗号加宽） |
